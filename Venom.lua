...
if Redis:get(Venom.."Venom:Game:CarName"..msg.chat_id) then
-- لازم يكون رد على رسالة
if msg.reply_to_message_id ~= 0 then
    -- نجيب الرسالة الأصلية اللي تم الرد عليها
    local MsgGame = LuaTele.getMessage(msg.chat_id, msg.reply_to_message_id)
    -- نتأكد إن الرسالة الأصلية موجودة ومن البوت نفسه
    if MsgGame and MsgGame.sender and MsgGame.sender.user_id == LuaTele.getMe().id then
        -- تحقق من صحة الإجابة
        if text == Redis:get(Venom.."Venom:Game:CarName"..msg.chat_id) then
            Redis:incrby(Venom.."Venom:Num:Add:Games"..msg.chat_id..msg.sender.user_id, 1)
            Redis:del(Venom.."Venom:Game:CarName"..msg.chat_id)
            return LuaTele.sendText(msg_chat_id, msg_id, "\n⌔︙لقد فزت في اللعبة ✅\n⌔︙للعب مرة أخرى، ارسل - سيارات", "md", true)
        else
            Redis:del(Venom.."Venom:Game:CarName"..msg.chat_id)
            return LuaTele.sendText(msg_chat_id, msg_id, "\n⌔︙لقد خسرت ❌ حظًا أوفر في المرة القادمة\n⌔︙للعب مرة أخرى، ارسل - سيارات", "md", true)
        end
    end
end
end
...
if text == "سيارات" then
if Redis:get(Venom.."Venom:Status:Games"..msg.chat_id) then
Redis:del(Venom.."Venom:Set:Cars"..msg.chat_id)

Cars_Rand = {
"مرسيدس","بي ام دبليو","تويوتا","كيا","هونداي","لكزس","شيفروليه","فورد",
"جيب","نيسان","هوندا","لامبورجيني","فيراري","بورش","بوغاتي","مازيراتي",
"رولز رويس","بنتلي","جاغوار","تسلا","ميتسوبيشي","سوزوكي","بيجو","فولكسفاجن",
"رينو","الفا روميو","استون مارتن","دودج","كاديلاك","جي ام سي","همر","لاند روفر",
"ساب","سوبارو","اودي","سيات","سكودا","فيات","ميني كوبر","تويوتا لاندكروزر"
}

car = Cars_Rand[math.random(#Cars_Rand)]
Redis:set(Venom.."Venom:Game:CarName"..msg.chat_id, car)

-- استبدال الاسم بالوصف
car = string.gsub(car,"مرسيدس","شركة ألمانية فاخرة معروفة بالفخامة والأمان")
car = string.gsub(car,"بي ام دبليو","شركة ألمانية تركّز على الأداء الرياضي والرفاهية")
car = string.gsub(car,"تويوتا","شركة يابانية شهيرة بالاعتمادية والتوفير")
car = string.gsub(car,"كيا","شركة كورية تقدم سيارات اقتصادية ومريحة")
car = string.gsub(car,"هونداي","شركة كورية بتصاميم حديثة وأسعار منافسة")
car = string.gsub(car,"لكزس","الفرع الفاخر لشركة تويوتا")
car = string.gsub(car,"شيفروليه","شركة أمريكية تجمع بين القوة والعملية")
car = string.gsub(car,"فورد","من أقدم الشركات الأمريكية بصناعة السيارات")
car = string.gsub(car,"جيب","شركة أمريكية متخصصة بالسيارات الرباعية")
car = string.gsub(car,"نيسان","شركة يابانية تجمع بين التقنية والسعر المناسب")
car = string.gsub(car,"هوندا","شركة يابانية مشهورة بالجودة والأداء")
car = string.gsub(car,"لامبورجيني","سيارة إيطالية رياضية فاخرة جداً")
car = string.gsub(car,"فيراري","أسطورة السيارات الرياضية الإيطالية")
car = string.gsub(car,"بورش","شركة ألمانية مشهورة بالسيارات الرياضية الفاخرة")
car = string.gsub(car,"بوغاتي","شركة فرنسية تصنع أسرع وأغلى سيارات بالعالم")
car = string.gsub(car,"مازيراتي","شركة إيطالية فاخرة ورياضية")
car = string.gsub(car,"رولز رويس","أفخم سيارة بريطانية على الإطلاق")
car = string.gsub(car,"بنتلي","سيارة بريطانية فاخرة ومميزة")
car = string.gsub(car,"جاغوار","شركة بريطانية تجمع بين الفخامة والسرعة")
car = string.gsub(car,"تسلا","شركة أمريكية رائدة في السيارات الكهربائية")
car = string.gsub(car,"ميتسوبيشي","شركة يابانية مشهورة بسيارات الدفع الرباعي")
car = string.gsub(car,"سوزوكي","شركة يابانية تصنع سيارات صغيرة واقتصادية")
car = string.gsub(car,"بيجو","شركة فرنسية تقدم سيارات اقتصادية")
car = string.gsub(car,"فولكسفاجن","شركة ألمانية مشهورة بسياراتها المتنوعة")
car = string.gsub(car,"رينو","شركة فرنسية تقدم تصاميم مبتكرة واقتصادية")
car = string.gsub(car,"الفا روميو","شركة إيطالية مشهورة بتصاميمها الجذابة")
car = string.gsub(car,"استون مارتن","شركة بريطانية فاخرة تظهر بأفلام جيمس بوند")
car = string.gsub(car,"دودج","شركة أمريكية بسيارات عضلية قوية")
car = string.gsub(car,"كاديلاك","شركة أمريكية فاخرة وفخمة")
car = string.gsub(car,"جي ام سي","شركة أمريكية مشهورة بسيارات البيك أب والـ SUV")
car = string.gsub(car,"همر","سيارة أمريكية ضخمة وقوية للطرق الوعرة")
car = string.gsub(car,"لاند روفر","شركة بريطانية متخصصة بالدفع الرباعي الفاخر")
car = string.gsub(car,"ساب","شركة سويدية سابقة بتقنيات مميزة")
car = string.gsub(car,"سوبارو","شركة يابانية مشهورة بنظام الدفع الرباعي")
car = string.gsub(car,"اودي","شركة ألمانية فاخرة وذات تقنيات متطورة")
car = string.gsub(car,"سيات","شركة إسبانية تتبع مجموعة فولكسفاجن")
car = string.gsub(car,"سكودا","شركة تشيكية تابعة لفولكسفاجن")
car = string.gsub(car,"فيات","شركة إيطالية تصنع سيارات صغيرة وعملية")
car = string.gsub(car,"ميني كوبر","سيارة بريطانية صغيرة بتصميم كلاسيكي مميز")
car = string.gsub(car,"تويوتا لاندكروزر","سيارة دفع رباعي قوية ومناسبة للطرق الوعرة")

return LuaTele.sendText(msg_chat_id, msg_id, "⌔︙اسرع واحد يكتب اسم السيارة بناءً على الوصف التالي:\n["..car.."]", "md", true)
end
end
if text == "معاني" then
if Redis:get(Venom.."Venom:Status:Games"..msg.chat_id) then
...
-- فتح القوائم الفرعية
if text == '🍕 بيتزا' then
    return sub_menu('بيتزا', '➕ اضافة طلب بيتزا', '➖ حذف طلب بيتزا')
end
if text == '🥤 جكسي' then
    return sub_menu('جكسي', '➕ اضافة طلب جكسي', '➖ حذف طلب جكسي')
end
if text == '🍕🥤 بيتزا و جكسي' then
    return sub_menu('بيتزا + جكسي', '➕ اضافة طلب بيتزا + جكسي', '➖ حذف طلب بيتزا + جكسي')
end
...
if text == '➕ اضافة طلب جكسي' then
    Redis:set(Venom.."wait_price:"..msg.sender.user_id, "juice")
    return LuaTele.sendText(msg_chat_id, msg_id, "⌔︙ارسل سعر وجبة *جكسي*", "md")
end
if text == '➕ اضافة طلب بيتزا + جكسي' then
    Redis:set(Venom.."wait_price:"..msg.sender.user_id, "both")
...
if Redis:get(Venom.."wait_price:"..msg.sender.user_id) then
    ...
    if not price then
        return LuaTele.sendText(msg_chat_id, msg_id, "⌔︙رجاءً ارسل السعر كـ رقم فقط ✅", "md")
    end
...
if text == '➖ حذف طلب جكسي' then
    Redis:set(Venom.."wait_del:"..msg.sender.user_id, "juice")
    return LuaTele.sendText(msg_chat_id, msg_id, "⌔︙ارسل سعر وجبة جكسي للحذف", "md")
end
if text == '➖ حذف طلب بيتزا + جكسي' then
    Redis:set(Venom.."wait_del:"..msg.sender.user_id, "both")
...
if Redis:get(Venom.."wait_del:"..msg.sender.user_id) then
    ...
    local list_key = Venom.."orders:list:"..order_type..msg.sender.user_id
    local removed = Redis:lrem(list_key, 1, tostring(price))
    Redis:del(Venom.."wait_del:"..msg.sender.user_id)
    if removed and removed > 0 then
        return LuaTele.sendText(msg_chat_id, msg_id, "⌔︙تم حذف طلب بسعر *"..price.."* دينار ✅", "md")
    else
...
if text == '📊 المجموع' then
    local function get_stats(order_type, icon, name)
        ...
        local count = #list
        local sum = 0
        for _, v in ipairs(list) do sum = sum + tonumber(v) end
...
if text == '♻️ تصفير الطلبات' then
    for _, item in ipairs({"pizza","juice","both"}) do
